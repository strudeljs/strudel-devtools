...
aliases[ALIAS_TYPES.SELECT_COMPONENT] = (data) => {
  chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { type: ALIAS_TYPES.SELECT_COMPONENT }, function(response) {
      // TBD: process the response which should include more data on selected component instance
    });
  });
